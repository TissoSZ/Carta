...
const block_left = parseFloat(window.getComputedStyle(block, null).left) || 0,
      block_top = parseFloat(window.getComputedStyle(block, null).top) || 0;
...
function Rise() {
    console.log("Iniciando animação de subida...");
    let timer2 = null,
        distance = 0;
    const target = 120,
          speed = 1;

    let love_top = parseFloat(window.getComputedStyle(love, null).top) || 0;
